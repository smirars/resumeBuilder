...
export default function LanguagesForm({ index, content }) {
  ...
  const handleBlur = (i, value) => {
    setErrors(prev => ({ ...prev, [i]: !value.trim() ? 'Поле не может быть пустым' : '' }))
  }
  ...
  return (
    <div className="block-form">
      ...
      <button className="block-form__add-btn" onClick={addItem}>+ Добавить язык</button>
    </div>
  )
}
